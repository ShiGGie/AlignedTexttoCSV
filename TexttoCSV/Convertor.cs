...
namespace CSVConverterConsole
{
    public class Convertor
    {
        ...
        public Convertor(TextFileMetadata input, CSVFileMetadata output)
        {
            this.textFile = input;
            this.csvFile = output;
        ...
        public void Run()
        {
            ...
            using (StreamReader readtext = new StreamReader(textFile.filePath))
            {
                ...
                while (!completed)
                {
                    #region while entire file has not been converted

                    fileNumber++;
                    if (fileNumber > 1)
                        fileNumString = String.Format("_{0}", fileNumber);

                    string newName2 = String.Format("{0}\\{1}{2}.csv", csvFile.filePath, newName, fileNumString);
                                ...
                                else if (lineIndex > csvFile.numHeaderLines - 1)
                                {
                                        ...
                                        (csvFile.dataFormat == 0 && ((string)dataList[csvFile.primaryKey]) != ""))
                                    {
                                        ...
                                        if (writtenBytes < 100000000)
                                            ...
                                            break; //break out of writing file
                                    }
